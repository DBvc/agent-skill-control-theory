...
这样可以避免理论随着外部仓库变化而过期，也避免你改进自己的 `dbvc/skills` 后，理论文档里的旧评论对不上现实。公开仓库只在 [docs/influences.md](docs/influences.md) 中作为启发来源被克制提及，不做逐仓库评价。

如果将来想写案例分析，建议放到单独的 `case-studies/` 目录，且每篇带日期，不写进核心理论。
